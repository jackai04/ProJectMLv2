...
def transition_model_alias(model_name, alias):
    ...
    try:
        ...
        # ตั้ง alias (แทน transition stage)
        print(f"Setting alias '{alias}' for model version {version_number}...")
        client.set_registered_model_alias(
            name=model_name, alias=alias, version=version_number
        )
        print(
            f"Successfully set alias '{alias}' for model '{model_name}' version {version_number}."
        )
